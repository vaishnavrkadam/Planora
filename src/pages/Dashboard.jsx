...
// Explicit city array mapped directly from your real CSV Firestore seeds
...
export default function Dashboard() {
  ...
  // These are just starter values! When you pick a new dropdown option, these update instantly.
  ...
  const handleGenerateClick = async () => {
    ...
    try {
      // Sends your exact selected dropdown value (e.g. "Delhi" or "Jaipur") straight to Firestore
      ...
      <Grid container spacing={4}>
        ...
        <Grid size={{ xs: 12, lg: 4 }}>
          <Paper sx={{ p: 4, display: 'flex', flexDirection: 'column', gap: 3.5 }}>
            ...
            {/* From & To Native Dropdown Pickers */}
            <Stack spacing={2.5}>
            ...
            <Box>
              ...
              <Box display="flex" justifyContent="space-between" sx={{ color: '#958ea0', fontSize: '11px', fontWeight: '700', letterSpacing: '0.05em', px: 0.5 }}>
                <Typography variant="caption" sx={{ fontSize: '10px' }}>BUDGET</Typography>
                <Typography variant="caption" sx={{ fontSize: '10px' }}>MID-RANGE</Typography>
                <Typography variant="caption" sx={{ fontSize: '10px' }}>LUXURY</Typography>
              </Box>
            </Box>

            {/* Days & Travelers Counters */}
                  ...
                  return (
                    <Chip 
                      ...
                      sx={{
                        ...
                        fontSize: '12px',
                        fontWeight: '500',
                        '&:hover': { backgroundColor: isSelected ? '#7c3aed' : 'rgba(255,255,255,0.05)' }
                      }}
                    />
                  );
                })}
            ...
            <Button 
              ...
              sx={{ 
                backgroundColor: '#8B5CF6', color: '#fff', py: 2, borderRadius: '16px', fontSize: '16px', fontWeight: '700',
                boxShadow: '0px 20px 40px rgba(139, 92, 246, 0.2)', '&:hover': { backgroundColor: '#7c3aed' }
              }}
            >
        ...
        <Grid size={{ xs: 12, lg: 8 }}>
          ...
          {!itinerary && !algoLoading && (
            <Paper sx={{ p: 6, textAlign: 'center', border: '1px dashed rgba(255,255,255,0.12)' }}>
              ...
              <Typography sx={{ color: '#958ea0' }}>Choose a destination from the dropdown on the left and tap generate to pull your live Firestore data.</Typography>
            </Paper>
          )}
              ...
              <Stack direction="row" spacing={1.5} sx={{ overflowX: 'auto', pb: 1, '&::-webkit-scrollbar': { display: 'none' } }}>
                {itinerary.days.map((day) => (
                  <Chip 
                    ...
                    sx={{
                      px: 1.5, py: 2, fontSize: '13px', fontWeight: '600', borderRadius: '9999px',
                      backgroundColor: selectedDay === day ? '#fff' : 'rgba(255,255,255,0.04)',
                      color: selectedDay === day ? '#0b1326' : '#cbc3d7',
                      '&:hover': { backgroundColor: selectedDay === day ? '#fff' : 'rgba(255,255,255,0.08)' }
                    }}
                  />
                ))}
              </Stack>

              <Box sx={{ position: 'relative', pl: 5, mt: 2 }}>
                ...
                {itinerary.timeline
                  ...
                  .map((item, idx) => (
                    <Box key={idx} sx={{ position: 'relative', mb: 4 }}>
                      ...
                      <Box sx={{ 
                        position: 'absolute', left: '-44px', top: '24px', transform: 'translateY(-50%)',
                        width: '10px', height: '10px', borderRadius: '50%', 
                        backgroundColor: '#8B5CF6', border: '3px solid #0b1326', boxShadow: '0 0 10px #8B5CF6',
                        zIndex: 2
                      }} />

                      <Paper sx={{ p: 3, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 3 }}>
                        <CardMedia
                          component="img"
                          image={item.image}
                          alt={item.title}
                          sx={{ width: { xs: '100%', sm: 120 }, height: 120, borderRadius: '16px', objectFit: 'cover' }}
                        />

                        <Box flex={1}>
                          <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={1}>
                            <Typography variant="caption" sx={{ color: '#958ea0', fontWeight: '700', letterSpacing: '0.05em' }}>
                              {item.time} — <span style={{ color: '#adc6ff' }}>{item.type}</span>
                            </Typography>
                            <Typography variant="h6" sx={{ fontWeight: '800', color: '#fff', fontSize: '18px' }}>
                              {item.cost}
                            </Typography>
                          </Box>

                          <Typography variant="h5" sx={{ fontSize: '20px', fontWeight: '700', mb: 1, color: '#fff' }}>
                            {item.title}
                          </Typography>
                          
                          <Typography variant="body2" sx={{ color: '#cbc3d7', mb: 2, lineHeight: 1.6 }}>
                            {item.desc}
                          </Typography>

                          <Stack direction="row" spacing={1}>
                            {item.tags.map((t, i) => (
                              <Chip key={i} label={t} size="small" sx={{ backgroundColor: 'rgba(139, 92, 246, 0.1)', color: '#d0bcff', fontSize: '11px' }} />
                            ))}
                          </Stack>
                        </Box>
                      </Paper>
